...
# Compute offset, the pelvis is not aligned with the world frame
x_offset = np.cos(-0.1)
y_offset = np.sin(-0.1)
...
for config, (file, folder, str_suffix) in enumerate(
    zip(file_name, [folder_KTC, folder_HTC, folder_NTC], ["KTC", "HTC", "NTC"])
):
    ...
    # # offset
    q[0, :] = q[0, :] + (config - 1) * x_offset
    q[1, :] = q[1, :] + (config - 1) * -y_offset
